...
class Track extends Database
{
	...
	public function readPaging($from_record_num,$records_per_page){
		$this->query('SELECT  tracks.id AS \'id\', albums.name AS \'album\', artists.name AS \'artist\',tracks.name AS \'name\', tracks.track_no AS \'track_no\', SEC_TO_TIME(tracks.duration) AS \'duration\',DATE_FORMAT(FROM_UNIXTIME(albums.release_date),\'%Y-%m-%d\') AS "release_date", tracks.mp3 as \'mp3\' FROM `tracks` Left JOIN albums ON tracks.album_id = albums.id left JOIN artists ON artists.id = albums.artist_id ORDER BY id LIMIT ?, ?');
